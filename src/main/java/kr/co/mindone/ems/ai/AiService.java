...
public class AiService {
        ...
        for (HashMap<String, Object> item3 : returnMapList3) {
            for (HashMap<String, Object> item2 : returnMapList2) {
                if (item3.get("PRDCT_TIME").toString().equals(item2.get("PRDCT_TIME").toString())) {
                    item3.put("TUBE_PRSR_PRDCT",
                            Double.parseDouble(item2.get("TUBE_PRSR_PRDCT").toString().replaceAll(",", "")));
                }
            }
    ...
    Map<String, Map<String, List<Map<String, Object>>>> selectWaterLevelTrend(Integer hour) {
        ...
        for (HashMap<String, Object> item : list) {
            String tagName = String.valueOf(item.get("TNK_GRP_NM"));
            Map<String, List<Map<String, Object>>> dateGroup = returnMap.computeIfAbsent(tagName,
                    k -> new LinkedHashMap<>());
                    ...
                    if (currentValue >= 2000) {
                        if (lastValidValue != null) {
                            ...
                            for (int j = i + 1; j < list.size(); j++) {
                                ...
                                String nextValueKey = nextItem.containsKey("value") ? "value"
                                        : (nextItem.containsKey("VALUE") ? "VALUE" : null);
    ...
    public List<HashMap<String, Object>> pumpPrdctSelectList(HashMap<String, Object> map) {
        ...
        if (false) {
            ...
            // }else if(wpp_code.equals("gs") || wpp_code.equals("ba") ||
            // wpp_code.equals("gr"))
        }
    ...
    public void setRandomList(List<HashMap<String, Object>> list) {
        ...
        if (list.size() > 6) {
            for (int i = 0; i < 7; i++) {
                ...
                // System.out.println("Random Adjustment within +/- 5% of A: " +
         ...
         * if(Double.parseDouble(stringObjectHashMap.get("PWR").toString()) != 0) {
         * double nowValue =
         * Double.parseDouble(stringObjectHashMap.get("PWR").toString()); // 주어진 값
         * double percentage = 5.0; // A의 5%를 계산하기 위한 퍼센테이지 값
    ...
    public List<HashMap<String, Object>> selectPwrPrdctList(HashMap<String, Object> map, int hour) {
        ...
        if (wpp_code.equals("hy") || wpp_code.equals("ji") || wpp_code.equals("ss") || wpp_code.equals("dev")
                || wpp_code.equals("seoul") || wpp_code.equals("ba")) {
        ...
        for (HashMap<String, Object> pumpItem : tempPrdctPumpList) {
            ...
            for (HashMap<String, Object> pumpPrdctItem : tempPumpList) {
                ...
                if (pumpPrdctPwrSumMap.containsKey(pumpPrdctItem.get("PRDCT_TIME").toString())) {
                    double all = Double
                            .parseDouble(pumpPrdctPwrSumMap.get(pumpPrdctItem.get("PRDCT_TIME").toString()).toString());
        ...
        if (wpp_code.equals("hy") || wpp_code.equals("ji") || wpp_code.equals("ss") || wpp_code.equals("dev")
            ...
            for (HashMap<String, Object> pwrAvgItem : tempPwrAvgHourList) {
                tempPwrAvgHourMap.put(pwrAvgItem.get("DATE").toString(),
                        Double.parseDouble(pwrAvgItem.get("PWR").toString()));
            }
            ...
            for (int i = 0; i < 24; i++) {
                ...
                if (pumpPrdctPwrSumMap.containsKey(checkDateTime) && tempPwrAvgHourMap.containsKey(checkTime)) {
                    ...
                    if (wpp_code.equals("gu")) {
                        ...
                        // 화성방식 계산
                        tempSum = (Double.parseDouble(tempPwrAvgHourMap.get(checkTime).toString())
                                - Double.parseDouble(pumpPrdctPwrSumMap.get(checkDateTime).toString())) * 0.75;
                    }

                    tempItem.put(checkDateTime,
                            tempSum + Double.parseDouble(pumpPrdctPwrSumMap.get(checkDateTime).toString()));
        ...
        // System.out.println("tempPwrPrdctCalHourList:" +
        // tempPwrPrdctCalHourList.toString());
    ...
    public List<HashMap<String, Object>> peakTimePrdctPwr(HashMap<String, Object> map) {
        ...
        for (HashMap<String, Object> prdctItem : pwrPrdctList) {
            // System.out.println("prdctItem.get(date).toString():" +
            // prdctItem.get("DATE").toString());
    ...
    public List<HashMap<String, Object>> selectPumpPrdctScadaList(HashMap<String, Object> map) {
        ...
        for (HashMap<String, Object> pwrItem : pwrList) {
            int timeSort = Integer
                    .parseInt(pwrItem.get("PRDCT_TIME").toString().split(" ")[1].split(":")[0].toString());
            for (int i = 0; i < emsTagList.size(); i++) {
                HashMap<String, Object> item = emsTagList.get(i);
                int nowSort = Integer.parseInt(item.get("SORT").toString()) - 1;
                if (nowSort == timeSort) {
    ...
    public List<HashMap<String, Object>> getEmergencyUse() {
        ...
        for (HashMap<String, Object> map : pumpMaster) {
            ...
            List<HashMap<String, Object>> emergencyWppCode = commonService
                    .selectWppTagCodeList("pumpAnlyOptStopOn" + pump_grp);
    ...
    public List<HashMap<String, Object>> getPumpAiUsage() {
        ...
        for (HashMap<String, Object> map : pumpMaster) {
            ...
            List<HashMap<String, Object>> emergencyWppCode = commonService
                    .selectWppTagCodeList("pumpAiOnOff" + pump_grp);
            // 그룹 별 1대1매칭이니 index 0 인것을 호출
            if (emergencyWppCode.size() == 1) {
                returnMap.put("tag", emergencyWppCode.get(0).get("TAG"));
    ...
    public HashMap<String, Object> pumpCommandStatus() {
        ...
        if (!pumpGrpList.isEmpty()) {
            if (wpp_code.equals("gs") || wpp_code.equals("gr") || wpp_code.equals("gu") || wpp_code.equals("ba")
                    || wpp_code.equals("dev") || wpp_code.equals("seoul")) {
    ...
    public HashMap<String, Object> pumpCommandAiControlStatus() {
        ...
        if (!pumpGrpList.isEmpty()) {
            ...
            /*
             * if(wpp_code.equals("gs") || wpp_code.equals("gu") || wpp_code.equals("ba") ||
             * wpp_code.equals("dev"))
             * {
             * return
             * pumpService.pumpCommandStatus(pumpService.selectAiPumpGrpListStr(PumpService.
             * AI_CONTROL));
             * }
             * else {
             * return
    ...
    public HashMap<String, Object> pumpChangeStatusAIModeGs(HashMap<String, Object> map) {
        ...
        if (Boolean.parseBoolean(pumpStatus.get("isChange").toString())
                && !Boolean.parseBoolean(pumpStatus.get("isRunning").toString())) {
            List<HashMap<String, Object>> changePumpList = pumpService.changePumpList(true,
                    pumpService.selectPumpGrpListStr());
        ...
        if (nowPwr == null || nowPwr.isEmpty()) {
            ...
            try {
                ...
                for (int time_diff = 0; time_diff < 24; time_diff++) { // time_diff를 0부터 시작
                    ...
                    String optIdx = String.format(wpp_code + ":%s-%s", dateFormat.format(prdctTimeCal.getTime())
                            .replace(":", "").replace("-", "").replace(" ", ""), timeStr);
     ...
     * - 변경 비교: cur_comb (TB_RAWDATA × PMB_TAG) vs pre_comb (TB_CTR_PUMPYN_RST +
     * INQUIRY)
